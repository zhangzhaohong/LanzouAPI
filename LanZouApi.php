<?php
    ...
    $ver = isset($_GET['ver']) ? $_GET['ver'] : "2";
    if ($ver != "1"){
        $url_new = "LanZouApi_20190823.php";
        if ($url != "")
            $url_new = $url_new . "?url=". $url;
        else{
            header("Location: LanZouApi_20190823.php");
            exit;
        }
        if ($pwd != "")
            $url_new = $url_new . "&pwd=" . $pwd;
        if ($type != "")
            $url_new = $url_new . "&type=" . $type;
